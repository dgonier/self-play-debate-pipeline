...
\begin{tabular}{llll}
\toprule
\textbf{Parameter} & \textbf{Phase 1 (ORPO)} & \textbf{Phase 3 (GRPO)} & \textbf{Iter3 Fix} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Training hyperparameters across phases. The Iter3 fix column shows
conservative parameters adopted after the GRPO regression.}
\end{table}
...
\begin{tabular}{lccl}
\toprule
\textbf{Dimension} & \textbf{Scale} & \textbf{GRPO Weight} & \textbf{Focus} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Judge scoring dimensions with GRPO weights.}
\end{table}

Normalized score: $(\sum w_i \cdot s_i / \sum w_i) - \text{length\_penalty} + \text{novelty\_bonus}$.
